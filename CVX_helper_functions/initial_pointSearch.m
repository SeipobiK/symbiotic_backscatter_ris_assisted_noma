function [V_opt,A_n_opt, B_n_opt, A_f_opt, B_f_opt, A_c_n_opt, B_c_n_opt,obj_prev,status] = initial_pointSearch(para,w_k,G_all, g_1_all,...
   ...
   N = para.N; % Number of RIS elements
               ...
               H_f_c{c} = diag(g_b_all{c}')*G_all*f2_all{c}*w_k(:, c);  
                ...
                %                                     (log2(exp(1)) / (B_c_prev_n(c) * (1 + A_c_prev_n(c) * B_c_prev_n(c)))) * (B_c_n(c) - B_c_prev_n(c));
                   ...
                   for j = 1:numClusters
                       ...
                       inv_pos(A_n(c))- real(trace(V  * H_n{c} * H_n{c}')) * alpha_n<=delta_p; % Near user

                        % inter cluster interference  + backscatter interference + noise power
                       delta_p>=inter_cluster_interference_near + ...
                                real(trace(V * H_n_c{c} * H_n_c{c}')) * eta_k + para.noise - B_n(c);

                        
                        inv_pos(A_f(c)) - real(trace(V * H_f{c} * H_f{c}')) * alpha_f <=delta_p; 
