function [V_opt,A_n_opt, B_n_opt, A_f_opt, B_f_opt, A_c_n_opt, B_c_n_opt,obj_history,obj_history_mc,converged] =passive_BF_opt(para,w_k,G_all, g_1_all,...
    ...
    step_size(1) = 0.00015; % More conservative
    ...
    for m = 2:max_iter
        ...
        % relax_parameter(m) = min(1, current_ratio + 0.4*(1-current_ratio)); % 10% step

        relax_parameter(m) = min(0.99999, current_ratio + step_size(m)); % 10% step
        disp(relax_parameter(m-1));
        disp(step_size(m-1));


        disp('---diff --');
        fprintf('%.10f',relax_parameter(m)-relax_parameter(m-1));
        disp('---diff --');
