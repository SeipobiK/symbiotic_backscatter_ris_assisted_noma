    ...
    g_2_all,g_b_all,f1_all,f2_all, alpha_n, alpha_f, Theta)
        ...
        %   G_all=G_all*para.scal; % Scale the channel matrix
           ...
           for c = 1:numClusters
                    ...
                    A_n(c) = abs(g_1_all{c}'*Theta*G_all*w_k(:, c)).^2 * alpha_n; % Near user


                    B_n(c) =inter_cluster_interference_near + ...
                            abs(g_b_all{c}'*Theta*G_all*f1_all{c}*w_k(:, c)).^2* para.eta + noise;

                        
                    A_f(c) = abs(g_2_all{c}'*Theta*G_all*w_k(:, c)).^2  * alpha_f;


                    B_f(c) = inter_cluster_interference_far + ...
                            abs(g_2_all{c}'*Theta*G_all*w_k(:, c)).^2  * alpha_n + ...
                            abs(g_b_all{c}'*Theta*G_all*f2_all{c}*w_k(:, c)).^2* para.eta + noise;
